...
.Sequence Diagram: Reset OnIdle With Ongoing Transaction
image::part2/images/figure_21a.svg[Sequence Diagram: Reset OnIdle With Ongoing Transaction]

.Sequence Diagram: Reset Immediate With Ongoing Transaction
...
|B12.FR.03 |If no _evseId_ is supplied +
  ...
  If any transaction is in progress and an OnIdle reset is received.
    |The transaction of the Charging Station SHALL be terminated normally, before the reboot, as in <<stop_transaction_options,E06 - Stop Transaction>>.
...
|B12.FR.07 |If an _evseId_ is supplied +
  ...
  If a transaction is in progress on the EVSE and an OnIdle reset is received.
    |The transaction on the EVSE SHALL be terminated normally, before the reset, as in <<stop_transaction_options,E06 - Stop Transaction>>.
