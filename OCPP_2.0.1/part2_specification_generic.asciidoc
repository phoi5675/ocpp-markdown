...
|[[ocpp_security_24]][24]   |Santesson, et al. "X.509 Internet Public Key Infrastructure Online Certificate Status Protocol - OCSP" RFC 6960. June 2013
...
|FR.05  |There are a few messages that do not provide their result in the response message, but send one or more messages that contain the result. When one of the following messages is received; GetReport, GetBaseReport, GetMonitoringReport, GetDisplayMessages, CustomerInformation, GetChargingProfiles, GetLog, UpdateFirmware, PublishFirmware, TriggerMessage(<message>)
  |The Charging Station SHALL acknowledge the requests in the list below with a response message(shown after the arrow "→") with the same _requestId_ as the request: +
  ...
  TriggerMessage(<message>) → <requested message>
    |The CSMS needs to know that a request for _requestId = X_ was accepted, so that it can expect result messages for this _requestId_. TriggerMessage does not have a _requestId_, but the requirement still applies in the sense that a TriggerMessageResponse must be sent before the sending the requested message.
|===
