...
|**Control Pilot signal** |Signal used by a Charge Point to inform EV of maximum Charging power or current limit, as defined by <<iec61851_1,[IEC61851-1]>>.
...
|<<reset,Reset>>                                                  |X | | | | |
|<<starttransaction,StartTransaction>>                            |X | | | | |
|<<statusnotification,StatusNotification>>                        |X | | | | |
|<<stoptransaction,StopTransaction>>                              |X | | | | |
...
[cols="1,10"]
|===
a|image::images/info_icon.png[width=30px] |A Charge Point MUST NOT send an Authorize.req before stopping a transaction if the presented idTag is the same as the idTag presented to start the transaction
|===
...
==3. Relation between Authorization Cache and Local Authorization List
...
==4. Unknown Offline Authorization
