...
|4 |설명 |하우징 내부에 신용/직불 카드 단말기가 내장된 충전기 또는 중앙 결제 단말기/키오스크가 있는 충전기 그룹에 속하는 충전기. EV 운전자가 카드를 사용하여 충전 비용을 지불합니다. 트랜잭션은 결제 회사에서 승인되고, CSMS는 결제 시스템으로부터 메시지를 받고, <<request_start_transaction_request,RequestStartTransactionRequest>>를 충전기로 보내 충전을 시작합니다.
